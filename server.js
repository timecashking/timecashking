...
import { CreditCardInvoiceSystem } from './credit-card-invoice-system.js';
import { ActivationSystem } from './activation-system.js';
...
app.post('/activation/activate', [
  body('code').notEmpty().withMessage('Código de ativação é obrigatório'),
  ...
  try {
    ...
    const result = await ActivationSystem.activateAccount(req.body);
    res.json(result);
  } catch (error) {
...
app.post('/activation/generate', authenticateToken, async (req, res) => {
  try {
    const { companyId } = req.body;
    const code = await ActivationSystem.generateActivationCode(companyId);
    res.json({ code });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
app.get('/activation/codes/:companyId', authenticateToken, async (req, res) => {
  try {
    const { companyId } = req.params;
    const codes = await ActivationSystem.listActivationCodes(companyId);
    res.json({ codes });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
app.post('/auth/reset-password', [
  ...
  try {
    ...
    const result = await AuthSystem.resetPassword(
      req.body.email, 
      req.body.newPassword
    );
...
app.post('/auth/link-google', authenticateToken, async (req, res) => {
  try {
    const { googleId, googleData } = req.body;
    
    if (!googleId || !googleData) {
      return res.status(400).json({ 
        success: false, 
        error: 'Dados do Google são obrigatórios' 
      });
    }

    const result = await AuthSystem.linkGoogleAccount(
      req.user.id, 
      googleId, 
      googleData
    );
...
app.post('/auth/unlink-google', authenticateToken, async (req, res) => {
  try {
    const result = await AuthSystem.unlinkGoogleAccount(req.user.id);
...
// Listar faturas
app.get('/credit-card-invoices', authenticateToken, async (req, res) => {
  try {
    const { status, accountId, year, month, page, limit } = req.query;
    
    const result = await CreditCardInvoiceSystem.listInvoices(req.user.companies[0].companyId, {
      status,
      accountId,
...
app.get('/credit-card-invoices/:id', authenticateToken, async (req, res) => {
  try {
    const result = await CreditCardInvoiceSystem.getInvoiceDetails(
      req.params.id, 
...
app.post('/credit-card-invoices', authenticateToken, [
  body('accountId').notEmpty(),
  body('billingMonth').isInt({ min: 1, max: 12 }),
  body('billingYear').isInt({ min: 2020, max: 2030 }),
  ...
  try {
    ...
    const result = await CreditCardInvoiceSystem.createInvoice({
      ...req.body,
...
// Fechar fatura
app.post('/credit-card-invoices/:id/close', authenticateToken, async (req, res) => {
  try {
    const result = await CreditCardInvoiceSystem.closeInvoice(
      req.params.id, 
      req.user.companies[0].companyId
    );
...
app.post('/credit-card-invoices/:id/pay', authenticateToken, [
  ...
  try {
    ...
    const result = await CreditCardInvoiceSystem.markAsPaid(
      req.params.id, 
      req.user.companies[0].companyId,
      req.body
    );
...
// Cancelar fatura
app.post('/credit-card-invoices/:id/cancel', authenticateToken, [
  ...
  try {
    ...
    const result = await CreditCardInvoiceSystem.cancelInvoice(
      req.params.id, 
      req.user.companies[0].companyId,
      req.body.reason
    );
...
app.post('/credit-card-invoices/generate-monthly', authenticateToken, [
  ...
  try {
    ...
    const result = await CreditCardInvoiceSystem.generateMonthlyInvoices(
      req.user.companies[0].companyId,
      req.body.month,
      req.body.year
    );
...
// Resumo de faturas para dashboard
app.get('/credit-card-invoices/summary', authenticateToken, async (req, res) => {
  try {
    const result = await CreditCardInvoiceSystem.getInvoiceSummary(
      req.user.companies[0].companyId
    );
...
app.get('/credit-card-invoices/history/:accountId', authenticateToken, async (req, res) => {
  try {
    const { months = 12 } = req.query;
    
    const result = await CreditCardInvoiceSystem.getCardInvoiceHistory(
      req.user.companies[0].companyId,
...
bot.command('start', ctx => {
  ctx.reply(
    'Bem-vindo ao TimeCash King! Use /vincular para conectar sua conta.'
  );
});
...
bot.command('vincular', async ctx => {
  ...
  try {
    ...
    ctx.reply(
      'Conta vinculada com sucesso! Agora você pode usar comandos como:\n\n' +
...
bot.on(message('text'), async ctx => {
  ...
  try {
    ...
    if (expenseMatch) {
      ...
      ctx.reply(
        `✅ Despesa registrada: R$ ${amount.toFixed(2)} - ${description}`
      );
      return;
    }
    ...
    if (incomeMatch) {
      ...
      ctx.reply(
        `✅ Receita registrada: R$ ${amount.toFixed(2)} - ${description}`
      );
      return;
    }
    ...
    const meetingMatch = text.match(
      /marcar reunião (?:dia )?(\d{1,2}) às (\d{1,2})h/i
    );
    if (meetingMatch) {
